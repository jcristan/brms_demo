...
/*    @In private QueueSender myQueueSender;   
    ...
    @Factory("customerLista")                                                               
    public void findCustomers()
    {
    	customerLista = entityManager.createQuery("from Customer c ").getResultList();
    	log.info("customerLista.size():"+customerLista.size());
    }
   ...
   public void findItems()
    {    	
    	if (lineaPedidoLista == null){ 
	    	lineaPedidoLista  = new ArrayList<LineaPedido>();
	    	List<Item> itemLista = entityManager.createQuery("from Item item ").getResultList();
	    	Iterator<Item> it = itemLista.iterator();
	    	while (it.hasNext()){
	        	LineaPedido lineaPedido = new LineaPedido();
	    		lineaPedido.setItem(it.next());
	    		lineaPedido.setQuantity(0);
	    		lineaPedidoLista.add(lineaPedido);
	    	}
    	}	
    }
